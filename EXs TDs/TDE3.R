...
col_names=c("ti","y_i_point_milieu","y(t_i)","Eg_i1/h",'y_i_implicite',"Eg_i2/h")  # ("Pas_i","sum El_i","max Eg_i","Ordre")
resultat_kable <- kable(result1,caption = 'Resultats numeriques obtenus pour un pas h = 0.1 sur [0,0.3] par les sch\'emas d\'Euler', "html", col.names = NULL,digits=4)%>%
  kable_styling(bootstrap_options = c("striped", "condensed", "bordered"))%>%
  add_header_above(col_names)%>%
  column_spec(1, bold = TRUE)%>%
  row_spec(1:nrow(result1), align = "c")
print(resultat_kable)
legend("topleft",legend = c("Courbe exacte","Courbe point milieu","Courbe implicite"), col = c("black","red","green"),lty = 1, cex = 0.8)
View(result1)