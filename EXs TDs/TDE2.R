...
resultat_kable <- kable(result1,caption = 'Resultats numeriques obtenus pour un pas h = 0.01 sur [0,2] par les sch\'emas d\'Euler', "html", col.names = NULL,digits=4)%>%
  kable_styling(bootstrap_options = c("striped", "condensed", "bordered"))%>%
  add_header_above(col_names)%>%
  column_spec(1, bold = TRUE)%>%
  row_spec(1:nrow(result1), align = "c")
print(resultat_kable)
legend("topleft",legend = c("Courbe exacte","Courbe explicite","Courbe implicite"), col = c("black","red","green"),lty = 1, cex = 0.8)
View(result1)